                                       ...
                                       size_t count, loff_t *ppos)
{
	...
	ret = tfa_recalibrate(tfa98xx_profile, tfa98xx_vsteps);
	mutex_unlock(&tfa98xx->dsp_lock);
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	if (!name)
		return -ENOMEM;
	scnprintf(name, MAX_CONTROL_NAME, "%s Profile", tfa98xx->fw.name);//application=mono---mono Profile
	pr_debug("control name  = %s\n",name);//TODO just need to deal here in audio hal;201610261833
	printk(KERN_ERR"control name  = %s\n",name);//TODO just need to deal here in audio hal;201610261833,// TODO: According to kernel mixer settings.
	...
	tfa98xx_controls[mix_index].get = tfa98xx_get_profile;//yangliang
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	printk(KERN_ERR"yangliang %s-1\n",__func__);

	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		pr_debug("Skipping tfa_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		printk(KERN_ERR"yangliang Skipping tfa_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		return;
	}

	if(tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
		pr_debug("Stream already started, skipping DSP power-on\n");
		printk(KERN_ERR"yangliang Stream already started, skipping DSP power-on\n");
		return;
	}
						...
						struct snd_soc_dai *dai)
{
	...
	if(substream)
		printk(KERN_ERR"yangliang %s-1.1 substream not null\n",__func__);

	if(!(substream->runtime))
		printk(KERN_ERR"yangliang %s-1.1.1 substream->runtime is null\n",__func__);
...
static int tfa98xx_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	printk(KERN_ERR"yangliang fmt=0x%x\n", fmt);
			     ...
			     struct snd_soc_dai *dai)
{
	...
	printk(KERN_ERR"yangliang Requested rate: %d, sample size: %d, physical size: %d\n",
			rate, snd_pcm_format_width(params_format(params)),
			snd_pcm_format_physical_width(params_format(params)));
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	printk(KERN_ERR"yangliang %s state: %d\n", __func__,mute);

	printk(KERN_ERR"yangliang %s-1\n",__func__);

	if (!(tfa98xx->flags & TFA98XX_FLAG_DSP_START_ON_MUTE)){
		printk(KERN_ERR"yangliang %s-1.1\n",__func__);
		return 0;
	}

	if (no_start) {
		printk(KERN_ERR"yangliang %s-1.1.1\n",__func__);
		pr_debug("no_start parameter set no tfa_start or tfa_stop, returning\n");
		return 0;
	}

	if (mute) {
		printk(KERN_ERR"yangliang %s-2 mute:%d\n",__func__,mute);
		...
		printk(KERN_ERR"yangliang %s-3 mute:%d\n",__func__,mute);
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	{
		...
		//.symmetric_channels = 1,//yangliang mask for voice spk using handset second mic uplink ok20170106
