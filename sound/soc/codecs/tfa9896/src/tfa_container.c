				...
				if ((vstep_msg_idx >= TFA_MAX_VSTEP_MSG_MARKER) || (vstep_msg_idx == j)) {
					...
					if(msgInfo->MessageType != 3) {
						if(modified == 1) {
							...
							err = dsp_msg(dev_idx, msgLength * 3,(const char *)msgInfo->CmdId);
							if (err != Tfa98xx_Error_Ok)
								return err;
						}
...
enum Tfa98xx_Error tfaContWriteProfile(int dev_idx, int prof_idx, int vstep_idx)
{
	...
	} else if((slave_address > 10)) { /* Ignore for softdsp */
		/* mute */
		tfaRunMute(dev_idx);
	...
	if (tfa98xx_cnt_verbose) {
		...
		/* Ignore for softdsp */
		if((tfa98xx_dev_family(dev_idx) == 2) && (slave_address > 10))
	...
	/* Ignore for softdsp */
	if ((prof->group != previous_prof->group || prof->group == 0) && (slave_address > 10)) {
		if(tfa98xx_dev_family(dev_idx) == 2)
