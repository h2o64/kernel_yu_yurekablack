...
TFASOFTDSP_NAMETABLE
TFASOFTDSP_BITNAMETABLE
...
char *tfaContBitName(uint16_t num, unsigned short rev)
{
	...
	case 0x12:
		...
		break;
	case 0x01:
		name =  tfa_bf2name(TfasoftdspBitNames, num);
		break;
	case 0x13:
...
char *tfaContDsName(uint16_t num, unsigned short rev)
{
	...
	case 0x12:
		...
		break;
	case 0x01:
		name =  tfa_bf2name(TfasoftdspDatasheetNames, num);
		break;
	case 0x13:
...
uint16_t tfaContBfEnum(const char *name, unsigned short rev)
{
	...
	case 0x12:
		...
		break;
	case 0x01:
		bfnum =  tfa_name2bf(TfasoftdspDatasheetNames, name);
		if (bfnum==0xffff)
			bfnum = tfa_name2bf(TfasoftdspBitNames, name);/* try long bitname table */
		break;
	case 0x13:
...
uint16_t tfaContBfEnumAny(const char *name)
{
	...
	if (bfnum!=0xffff)
		return bfnum;
	bfnum = tfa_name2bf(TfasoftdspBitNames, name);
	if (bfnum!=0xffff)
		return bfnum;
