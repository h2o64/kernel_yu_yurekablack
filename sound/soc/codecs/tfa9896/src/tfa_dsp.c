...
static void tfa_set_query_info (int dev_idx)
{
	...
	default:
		/* This is a softDSP section for the tfa9872 device */
		if (handles_local[dev_idx].rev < 10) {
			/* tfa9872 */
			handles_local[dev_idx].tfa_family = 2;
			handles_local[dev_idx].spkr_count = 1;
			handles_local[dev_idx].daimap = Tfa98xx_DAI_TDM;
...
tfa_probe(unsigned char slave_address, Tfa98xx_handle_t *pHandle) {
	...
	*pHandle = -1;
	...
	/* If the slave address is < 10 then overload for mmap read/write */
	/* We need to do this here otherwise we cannot read the REV id */
#if !defined(__KERNEL__)
	if(slave_address < 10)
	{
		if (handles_local[idx].dev_ops.dsp_msg == NULL)
			handles_local[idx].dev_ops.dsp_msg = mmap_dsp_msg;
		if (handles_local[idx].dev_ops.dsp_msg_read == NULL)
			handles_local[idx].dev_ops.dsp_msg_read = mmap_dsp_msg_read;
		if (handles_local[idx].dev_ops.reg_read == NULL)
			handles_local[idx].dev_ops.reg_read	= mmap_read_register16;
		if (handles_local[idx].dev_ops.reg_write == NULL)
			handles_local[idx].dev_ops.reg_write = mmap_write_register16;
		if (handles_local[idx].dev_ops.mem_read == NULL)
			handles_local[idx].dev_ops.mem_read = mmap_dsp_read_mem;
		if (handles_local[idx].dev_ops.mem_write == NULL)
			handles_local[idx].dev_ops.mem_write = mmap_dsp_write_mem_word;
	}
#endif
	...
	case 0x6C:
	case 0x6E:
	case 0x00: /* This is the register side of the mmap for the softDSP section for the 72 */
	case 0x04: /* This is the xmem side of the mmap for the softDSP section for the 72 */
                                   ...
                                   uint16_t mask)
{
	...
	/* For softDSP we write to MMAP, not MTP */
	if((handles_local[handle].rev & 0xff) == 0x01) {
		if(mask == 0x02) { /* MTPEX */
			reg_write(handle, 10, value);
		} else if(mask == 0x01) { /* MTPOTC */
			reg_write(handle, 7, value);
		}
...
static enum Tfa98xx_Error tfa98xx_aec_output(Tfa98xx_handle_t handle, int enable)
{
	...
	if (handles_local[handle].slave_address < 10) /* SoftDSP */
		return err;
...
enum Tfa98xx_Error show_current_state(Tfa98xx_handle_t handle)
{
	...
	/* The softdsp has no manager state! */
	if (handles_local[handle].slave_address < 10)
		return err;
...
enum Tfa98xx_Error tfaRunSpeakerBoost(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	if ((value == 1) && (!istap_prof)) {
		...
		} else { /* Only for softDSP */
#ifndef __KERNEL__
			/* If MTPEX is set then write the calibration values to the mmap */
			err = tfaSetCalibrationValues(handle, 0, 0);
#endif
		}
...
enum Tfa98xx_Error tfaRunSpeakerStartup(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	if ( !force ) { // in case of force CF already runnning
		...
		if(handles_local[handle].slave_address > 10) {
			...
			/* We have to write the (I2C) settings for softDSP! */
			pr_debug("\n----- Device (I2C) settings softDSP -----\n");
			err = tfaContWriteRegsDev(handle); // write device register settings
			pr_debug("----- Profile (I2C) settings softDSP -----\n");
	...
	/* For softDSP we should not write files when we perform a remote start */
	/* Linux kernel does not know about softDSP */
...
#ifndef __KERNEL__
/*
 * Write calibration values for softDSP
 */
enum Tfa98xx_Error tfaSetCalibrationValues(Tfa98xx_handle_t handle, uint16_t Re25C_L, uint16_t Re25C_R)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	unsigned char bytes[6] = {0};
	static int set_attempt = 0;
	uint16_t tfa72_MTPEX;
	int ReL, ReR;

	if (handles_local[handle].slave_address < 10) {
		/* Read from mmap to see the current status of calibration */
		err = reg_read(handle, 10, &tfa72_MTPEX);

		/* If calibration is already done before perform a SetRe25C */
		if (tfa72_MTPEX == 1) {
			/* Get the values from the MMAP */
			err = reg_read(handle, 8, &Re25C_L);
			err = reg_read(handle, 9, &Re25C_R);

			/* For mono configuration copy the Primary to Secondary */
			if(Re25C_L > 0 && Re25C_R == 0)
				Re25C_R = Re25C_L;

			/* Shift 6 bit before writing to DSP */
			ReR = (Re25C_R * TFA2_FW_ReZ_SCALE) / 1000;
			ReL = (Re25C_L * TFA2_FW_ReZ_SCALE) / 1000;

			bytes[0] = (uint8_t) ((ReL >> 16) & 0xffff);
			bytes[1] = (uint8_t) ((ReL >> 8) & 0xff);
			bytes[2] = (uint8_t) (ReL & 0xff);
			bytes[3] = (uint8_t) ((ReR >> 16) & 0xffff);
			bytes[4] = (uint8_t) ((ReR >> 8) & 0xff);
			bytes[5] = (uint8_t) (ReR & 0xff);

			if(set_attempt == 0) {
				pr_info("SetRe25C(0x%x , 0x%x) \n", ReL, ReR);
				set_attempt++;
			}
			err = tfa_dsp_cmd_id_write(handle, MODULE_SPEAKERBOOST, SB_PARAM_SET_RE25C, sizeof(bytes), bytes);
		} else if (Re25C_L > 0) {
			/* GetRe25C is compleet, write values into mmap so that the next time calibration is not done again */
			err = reg_write(handle, 10, 1); //MTPEX is 1
			err = reg_write(handle, 8, Re25C_L);
			err = reg_write(handle, 9, Re25C_R);
		}
	}
...
/*
 * Get calibration values for softDSP
 */
enum Tfa98xx_Error tfaGetCalibrationValues(Tfa98xx_handle_t handle)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaSpeakerFile_t *spkr;
	int cal_value, nr_bytes=6; /* 6 = 2 speakers * 3 bytes */
	unsigned char bytes[6] = {0};
	int cal_data[2] = {0};
	static int cal_attempt = 0, typical_cal_value=0;

	if (handles_local[handle].slave_address < 10) {
		if(cal_attempt < 100) {
			if (tfa98xx_runtime_verbose)
				pr_info("Get calibration values from softDSP \n");
			/* calibrate */
			err = tfa_dsp_cmd_id_write_read(handle,MODULE_SPEAKERBOOST,SB_PARAM_GET_RE25C, nr_bytes, bytes);
			if (err == Tfa98xx_Error_Ok) {
				tfa98xx_convert_bytes2data(nr_bytes, bytes, cal_data);
				cal_value = (1000 * cal_data[0]) / TFA2_FW_ReZ_SCALE;
				pr_info("Calibration value = %d mOhm \n", cal_value);

				/* Only do this once */
				if(typical_cal_value == 0) {
					/* Get the resistance from the speaker file header */
					spkr = (nxpTfaSpeakerFile_t *)tfacont_getfiledata(handle, 0, speakerHdr);
					if(spkr != 0)
						typical_cal_value = spkr->ohm_primary * 1000;
					else
						typical_cal_value = 8000; /* Assume 8 ohm */
				}

				/* For now the range is 50% offset! */
				if ((cal_value < (typical_cal_value/100 * 50)) || (cal_value > (typical_cal_value/100 * 150))) {
					pr_info("Calibration value (%d) is not within expected (%d) range! \n", cal_value, typical_cal_value);
					err = Tfa98xx_Error_RpcCalibFailed;
				} else {
					cal_data[0] = (1000 * cal_data[0]) / TFA2_FW_ReZ_SCALE;
					cal_data[1] = (1000 * cal_data[1]) / TFA2_FW_ReZ_SCALE;
					err = tfaSetCalibrationValues(handle, (uint16_t)cal_data[0], (uint16_t)cal_data[1]);
					if (err) pr_info("Writing values to MMAP Failed! \n");
				}
			}

			if (err == Tfa98xx_Error_RpcCalibFailed) { /* Calibration Failed from DSP */
				if(cal_attempt == 0)
					pr_info("Calibration Failed! Retrying.. \n");
				else if(cal_attempt == 10 || cal_attempt == 30 || cal_attempt == 50 || cal_attempt == 70)
					pr_info("Calibration still in progress.. \n");

				msleep_interruptible(10); /* wait 10ms to avoid busload */
				cal_attempt++;
				/* MTPEX is set to 1 to trigger SetRe25C to write the values */
				err = reg_write(handle, 8, 0);
				err = reg_write(handle, 9, 0);
				err = reg_write(handle, 10, 1);
				/* Do a SetRe25C and trigger a re-calibration */
				err = tfaSetCalibrationValues(handle, 0, 0);
				/* MTPEX is set back to 0 to trigger GetRe25C */
				err = reg_write(handle, 10, 0);
			}
		} else {
			pr_info("Calibration Failed! Restart the whole alspro process! \n");
...
enum Tfa98xx_Error tfaRunStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	if (tfa_cf_enabled(handle)) {
		...
		if (tries == CFSTABLE_TRIES) {
			...
			return Tfa98xx_Error_StateTimedOut;
		}  else if (tfa98xx_runtime_verbose) pr_debug(" OK (tries=%d)\n", tries);
	}
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for( dev=0; dev < devcount; dev++) {
#ifdef __KERNEL__
		/* ignore SoftDSP in kernel driver */
		if (handles_local[dev].slave_address < 10)
			continue;
#endif
		err = tfaContOpen(dev);
		if ( err != Tfa98xx_Error_Ok)
		 ...
		 */
		if(((handles_local[dev].rev & 0xff) != 0x72) && (handles_local[dev].slave_address > 10)) {
	...
	for( dev=0; dev < devcount; dev++) {
		...
		/* Dont run this for softdsp or 72 */
		if(((handles_local[dev].rev & 0xff) != 0x72) && (handles_local[dev].slave_address > 10)) {
			if (cal_profile >= 0) {
				next_profile = 0;
...
tfa_dsp_get_calibration_impedance(Tfa98xx_handle_t handle) {
	...
	if (error == Tfa98xx_Error_Ok)
	{
		...
		} else if ((handles_local[handle].rev & 0xff) == 0x72) {
			if (tfa98xx_runtime_verbose)
				pr_debug("Getting calibration values from MTP\n");
			handles_local[handle].mohm[0] = tfa_read_reg(handle, (uint16_t)TFA_MK_BF((0xF5),0,16));
		} else if((handles_local[handle].rev & 0xff) == 0x01) {
			if (tfa98xx_runtime_verbose)
				pr_debug("Getting calibration values from MMAP\n");
			for(i=0; i<spkr_count; i++) {
				handles_local[handle].mohm[i] = tfa_read_reg(handle, (uint16_t)TFA_MK_BF((8+i),0,16));
			}
		} else {
...
int tfa_get_swprof(Tfa98xx_handle_t handle)
{
	...
	if ( handles_local[handle].profile < 0) {
		...
		if ((handles_local[handle].rev & 0xff) == 0x72)
			...
			reg_read(handle, TFA_SOFTDSP_BF_SWPROFIL, &swprofil);
			handles_local[handle].profile = (int)swprofil - 1;
		} else
...
int tfa_set_swprof(Tfa98xx_handle_t handle, unsigned short new_value)
{
	...
	if ((handles_local[handle].rev & 0xff) == 0x72) {
		...
		reg_write(handle, TFA_SOFTDSP_BF_SWPROFIL, new_value+1);
	} else if ( handles_local[handle].tfa_family > 1 ) {
...
int tfa_get_swvstep(Tfa98xx_handle_t handle)
{
	...
	if ((handles_local[handle].rev & 0xff) == 0x72)
		value = tfa_get_bf(handle, TFA9872_BF_SWVSTEP); /* get current vstep[0] */
	else
...
{
	int value;
	unsigned short cold_started=0;

	if((handles_local[handle].rev & 0xff) == 0x72) {
		/* If device is already in operating state then it has to be warm! */
		if(TFA_GET_BF(handle, MANSTATE) == 9)
			return 0; //warm
		else
			return 1; //cold
	} else if((handles_local[handle].slave_address < 10)) {
		//cf_cold_started
		reg_read(handle, 11, &cold_started);
		value = (int)cold_started;
	} else {
...
enum Tfa98xx_Error tfa98xx_set_boost_trip_level()
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	int dev, devcount = tfa98xx_cnt_max_device();
	unsigned short calibrate_done=0, Re25C=0, local_open=0;

	/* No matter what order the device sections are we always need to check mmap first! */
	for(dev=0; dev < devcount; dev++) {
		if (handles_local[dev].slave_address < 10) {
#ifdef __KERNEL__
			/* ignore SoftDSP in kernel driver */
			if (handles_local[dev].slave_address < 10)
				continue;
#endif
			/* Only do this when calibration is already done, otherwise we do it twice! */
			error = reg_read(dev, 10, &calibrate_done);
			if(calibrate_done == 0)	{
				return error;
			}

			/* Read the calibration value from MMAP */
			error = reg_read(dev, 8, &Re25C);
		}
	}

	for(dev=0; dev < devcount; dev++) {
		/* Only for 72 (softdsp is also registered as 72, so check for slave > 10) */
		if (handles_local[dev].slave_address > 10) {
			...
			if(Re25C == 0) {
				Re25C = (unsigned short)tfa_read_reg(dev, (uint16_t)TFA_MK_BF((0xF5),0,16));
			}
...
enum Tfa98xx_Error tfa98xx_adapt_boost_trigger_level(Tfa98xx_handle_t handle, int set_value)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;

	/* Only for 72 (to write HW register)
	 * Softdsp is also registered as 72, so check for slave > 10
	 */
	if (handles_local[handle].slave_address > 10) {
		if (handles_local[handle].dev_ops.tfa_adapt_boost_trigger_level)
			error = (*handles_local[handle].dev_ops.tfa_adapt_boost_trigger_level)(handle, set_value);
	}
