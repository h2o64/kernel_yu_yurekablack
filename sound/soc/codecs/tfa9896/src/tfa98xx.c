...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->flags & TFA98XX_FLAG_CALIBRATION_CTL)
		nr_controls += 1; /* calibration control */
 ...
 */
static char *fw_name = "tfa9896_v3.cnt";
...
static void tfa98xx_dsp_init_work(struct work_struct *work)
{
	...
	pr_info("%s: handle = %d\n", __func__, tfa98xx->handle);

	/* Only do dsp init for master device */
	if (tfa98xx->handle != 0) {
		pr_info("not master handle = %d, Only do dsp init for master device\n", tfa98xx->handle);
		return;
	}
                             ...
                             struct snd_soc_dai *dai)
{
	...
	if (prof_idx < 0) {
		pr_info("invalid sample rate %d.\n", rate);
		rate = 48000;
		pr_info("set force sample rate = %d.\n", rate);
	}
